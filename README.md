...
[Bowtie2](http://bowtie-bio.sourceforge.net/bowtie2/index.shtml) (version: 2.5.4),
git.
...
- **Contigs from de novo assembly:** &nbsp; Assembled contig sequences produced by short-read or hybrid assembly pipelines. 
...
-m, --method
 ...
 alignment. Supported methods include BLAT, Minimap2sam (SAM output), and
 Minimap2paf (PAF output). If the input file is in BAM format, it is
 recommended to convert it to SAM format before analysis.

-i, --input_file
 Alignment files in SAM, PAF, or PSL format generated by alignment tools.
 This serves as the primary input for DenovoFusion's analysis.

-a, --input_assembly
 The contig file from de novo assembly in FASTA format. When using PSL mode,
 please use the processed (cut) contigs instead of the original contigs.

-o, --output
 Output file path, which will include separate files for fusion genes,
...
-1, --fastq1
 FASTQ file for forward reads (e.g. 01.fastq.gz). Multiple input files can be
 separated by commas, as in Bowtie2 input format.

-2, --fastq2
 FASTQ file for reverse reads (e.g. 02.fastq.gz). Multiple input files can be
 separated by commas, as in Bowtie2 input format.

-p, --prefix
 Prefix for all temporary and result files; you can use the sample name here.
...
-c, --max_alignment_count
 Maximum number of alignments considered for each contig.
 Larger values may increase processing time (Default: 100).
...
-v, --coverage-differ
 Coverage ratio threshold between adjacent bases in breakpoint prediction
 (Default: 0.85).
...
-l, --max-overlap-size
 Maximum number of overlapping bases allowed in paired alignments (Default: 8).

-f, --max-gap-size
 Maximum number of gap bases allowed in paired alignments (Default: 2).

-E, --min-edge-length
 Minimum edge length of split reads in re-alignment; adjust according to
 read length (Default: 20).

-I, --inclusion-fraction-weight
 Weight of the inclusion fraction in alignment score calculation (Default: 1).

-O, --overlap-fraction-weight
 Weight of the overlap fraction in alignment score calculation (Default: 1).

-S, --size-weight
 Weight of alignment count in alignment score calculation (Default: 1).

-T, --long-gap-threshold
 Gap threshold for candidate fusions on the same chromosome (Default: 200000).
